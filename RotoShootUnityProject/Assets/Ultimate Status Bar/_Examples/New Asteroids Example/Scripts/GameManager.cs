...
namespace UltimateStatusBarExample.Spaceship
{
	public class GameManager : MonoBehaviour
	{
		...
		public GameObject asteroidHealthBar;
		public LayerMask healthBarBlockingMask;
		...
		void SpawnHealthPickup ()
		{
			...
			// Create a Vector3 variable to store the spawn position.
			Vector3 pos = Vector3.zero;

			// If the X value of the spawn direction is greater than the Y, then spawn the health pickup to the left or right of the screen, determined by the value of the direction variable.
			...
			// Else the Y value is greater than X, so spawn the health pickup up or down, determined by the value of the direction variable.
			else
		...
		void SpawnAsteroid ()
		{
			...
			// If the X value of the spawn direction is greater than the Y, then spawn the asteroid to the left or right of the screen, determined by the value of the direction variable.
			if( Mathf.Abs( dir.x ) > Mathf.Abs( dir.y ) )
				pos = new Vector3( Mathf.Sign( dir.x ) * Camera.main.orthographicSize * Camera.main.aspect * 1.3f, 0, dir.y * Camera.main.orthographicSize * 1.2f );
			// Else the Y value is greater than X, so spawn the asteroid up or down, determined by the value of the direction variable.
			else
				pos = new Vector3( dir.x * Camera.main.orthographicSize * Camera.main.aspect * 1.3f, 0, Mathf.Sign( dir.y ) * Camera.main.orthographicSize * 1.2f );
			...
			GameObject ahb = Instantiate( asteroidHealthBar, FindObjectOfType<Canvas>().transform );
			ahb.SetActive( true );

			UltimateStatusBar ahbs = ahb.GetComponent<UltimateStatusBar>();
			ast.GetComponent<AsteroidController>().healthBar = ahbs;
			ahbs.StartFollowTransform( ast.transform, Camera.main, new Vector3( 0, 0, 12 ), healthBarBlockingMask );
		}
		...
		public void SpawnDebris ( Vector3 pos )
		{
			...
			{
				// Create a force to make the debris fly away from each other.
