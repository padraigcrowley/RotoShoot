...
public class WaypointManagerEditor : Editor 
{
    ...
    void DrawWaypoint()
    {
        ...
        foreach (var point in script.selected.points)
        {
          ...
          Handles.SphereHandleCap(0, point, Quaternion.identity, script.selected.pointSize, EventType.Repaint);
    ...
    void DrawHandlePoint()
    {
        ...
        for (int i = 0; i < wayPoints.Count; i++)
        {
            ...
            wayPoints[i] = Handles.FreeMoveHandle(wayPoints[i], Quaternion.identity, script.selected.pointSize, Vector3.zero, Handles.SphereHandleCap);
            ...
            if (script.selected.lineType == PathLineType.BezierCurve)
            {
                ...
                if (i != 0)
                {
                    Vector3 movedPoint = Handles.FreeMoveHandle(firstControlPoint, Quaternion.identity, script.selected.pointSize, Vector3.zero, Handles.SphereHandleCap);
                ...
                {
                    Vector3 movedPoint = Handles.FreeMoveHandle(secondControlPoint, Quaternion.identity, script.selected.pointSize, Vector3.zero, Handles.SphereHandleCap);
